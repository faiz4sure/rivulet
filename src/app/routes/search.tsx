...
const AVAILABLE_PROVIDERS = ["dummyProvider", "movieboxProvider"];
...
export function SearchPage() {
  ...
  const [providerResults, setProviderResults] = useState<ProviderResult[]>(
    AVAILABLE_PROVIDERS.map(p => ({ provider: p, results: null, loading: true }))
  );

  useEffect(() => {
    if (!query) return;

    setProviderResults(AVAILABLE_PROVIDERS.map(p => ({ provider: p, results: null, loading: true })));

    AVAILABLE_PROVIDERS.forEach(async (provider) => {
      try {
        const runner = new PluginRunner("dummy-plugin", "../dummy/index.ts");
        await runner.installDependencies();
        
        const results = await runner.search(provider, query, 1);
        if ((results as any).error) {
            throw new Error((results as any).message || "Unknown error");
        }
        
        setProviderResults(prev => prev.map(p => 
          p.provider === provider ? { ...p, results, loading: false } : p
        ));
      } catch (err) {
        setProviderResults(prev => prev.map(p => 
          p.provider === provider ? { ...p, error: String(err), loading: false } : p
        ));
      }
    });
  }, [query]);
        ...
        {providerResults.map((pr) => (
          <div key={pr.provider} className="space-y-4 bg-neutral-900/20 p-6 rounded-3xl border border-neutral-800/60 shadow-xl backdrop-blur-sm">
            <div className="flex items-center gap-3 mb-6 px-2">
              ...
              <h2 className="text-2xl font-semibold text-white capitalize">{pr.provider}</h2>
              {pr.loading && (
